...
namespace OurLittleTastyServerBot.Classes.Observers
{
    public class DatabaseWorker : AbstractObserver<EventArgs>
    {
        ...
        private static Result<UpdateRecord> Convert(Update value)
        {
            try
            {
                var time = UnixDateTime.FromUnixFormat(value.message.UtcDateTimeInUnitFormat);
                    
                var result = UpdateRecord.Factory.CreateEmpty(value.update_id, value.message.message_id, time,
                    value.message.text, value.message.chat.id, value.message.from.id, value.message.from.first_name);
